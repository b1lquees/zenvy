...
import Sidebar, { PageLayout, useIsMobile, T, s } from '../../components/Sidebar'
  ...
  const BookingPanel = () => (
    <div style={{
      ...
      <div style={{
        ...
        fontWeight: 600, color: T.ink, marginBottom: 16
      }}>
        Book this Service
      </div>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <select value={selectedProvider} onChange={e=>handleProviderSelect(e.target.value)} style={{...s.select,width:'100%',boxSizing:'border-box'}}>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <input type="date" value={form.service_date} onChange={e=>setForm({...form,service_date:e.target.value})} style={{...s.input,width:'100%',boxSizing:'border-box'}} />
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <input placeholder="Your address" value={form.location} onChange={e=>setForm({...form,location:e.target.value})} style={{...s.input,width:'100%',boxSizing:'border-box'}} />
      </div>
      ...
      <button onClick={handleBook} disabled={booking} style={{...s.btn,width:'100%',justifyContent:'center',opacity:booking?0.6:1,boxSizing:'border-box'}}>
    ...
    <div style={s.page}>
      ...
      <PageLayout>
        ...
        <div style={{ marginBottom:22, paddingBottom:16, borderBottom:`2px solid ${T.mixBorder}` }}>
          <button onClick={()=>navigate('/customer/browse')} style={{...s.btnGhost,marginBottom:12,fontSize:12}}>← Back to Services</button>
          ...
          <div style={{
            ...
            <div>
              <span style={s.badge({bg:T.bluePale,color:'#1a4870',border:T.blueMid})}>
                {service.category?.category_name||'General'}
              </span>
              <h1 style={{...s.h1, marginTop:8, fontSize: isMobile ? 22 : 26}}>
                {service.service_name}
              </h1>
              {avgRating && (
                <div style={{color:'#d97706',fontSize:14,marginTop:4,fontWeight:700}}>
        ...
        <div style={{
          ...
          <div>
            {/* About */}
            <div style={s.card}>
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:600,color:T.ink,marginBottom:12}}>
                About this Service
              </div>
              <p style={{fontSize:14,color:T.muted,lineHeight:1.8,fontWeight:600,margin:0}}>
            ...
            {providers.length>0 && (
              <div style={{...s.card,marginTop:14}}>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:600,color:T.ink,marginBottom:14}}>
                  Available Providers
                </div>
                {providers.map(p=>(
                  <div key={p.user_id} style={{
                    ...
                    justifyContent:'space-between',
                    padding:'12px 0',
                    borderBottom:`1.5px solid ${T.mixPale}`,
                    gap: 8,
                  }}>
                    <div style={{display:'flex',alignItems:'center',gap:10,minWidth:0}}>
                      <div style={{...s.avatar(isMobile?34:40),fontSize:isMobile?12:14,flexShrink:0}}>
                        {p.users?.first_name?.charAt(0)}
                      </div>
                      <div style={{minWidth:0}}>
                        <div style={{fontWeight:800,fontSize:isMobile?13:14,color:T.ink}}>
                          {p.users?.first_name} {p.users?.last_name}
                        </div>
                        <div style={{fontSize:11,color:T.muted,fontWeight:600}}>
                          {p.experience_years} yrs experience · ★ {p.average_rating||'New'}
                        </div>
                      </div>
                    </div>
                    <span style={{...s.badge(p.availability_status
                  ...
                  <div key={r.review_id} style={{marginBottom:14,paddingBottom:14,borderBottom:`1.5px solid ${T.mixPale}`}}>
                    <div style={{display:'flex',justifyContent:'space-between',marginBottom:6,gap:8,flexWrap:'wrap'}}>
                      ...
                      <div style={{color:'#d97706',flexShrink:0}}>
                        {'★'.repeat(r.rating)}<span style={{color:T.mixBorder}}>{'★'.repeat(5-r.rating)}</span>
                      </div>
                    </div>
